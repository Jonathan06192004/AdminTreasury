...
function selectMission(mission) {
  ...
  document.getElementById('table-title').textContent = `Offerings — ${mission.code}`;
...
async function handleSubmit(e) {
  ...
  let error;
  if (id) {
    ({ error } = await dbData.from('offerings').update({ year, month, church_id: churchId, amount, budget }).eq('id', id));
  } else {
    ({ error } = await dbData.from('offerings').insert({ year, month, church_id: churchId, amount, budget }));
  }
...
async function confirmDelete() {
  ...
  const { error } = await dbData.from('offerings').delete().eq('id', deleteTargetId);
  closeDelete();
  if (!error) loadData();
}
