...
async function handleSubmit(e) {
  ...
  let error;
  if (id) {
    ({ error } = await dbData.from('offerings').update({ year, month, amount, budget }).eq('id', id));
  } else {
    ({ error } = await dbData.from('offerings').insert({ mission_id: activeMissionId, year, month, amount, budget }));
  }
